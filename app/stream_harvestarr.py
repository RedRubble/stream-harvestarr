...
SCANINTERVAL = 60
DEFAULT_RELEASE_GROUP = 'StreamHarvestarr'
...
INHERITABLE_KEYS = frozenset((
    ...
    'strict_parts', 'extract_flat', 'release_group',
))
...
class StreamHarvester(object):
        ...
        for wnt in self.series:
            ...
            for ser in candidate_series:
                if 'sonarr_id' in wnt or normalize_title(wnt['title']) == normalize_title(ser['title']):
                    ...
                    ser['extract_flat'] = True
                    ser['release_group'] = DEFAULT_RELEASE_GROUP
                    # Update values
                    if 'regex' in wnt:
                        regex = wnt['regex']
                        if 'sonarr' in regex:
                    ...
                    if 'extract_flat' in wnt:
                        ser['extract_flat'] = wnt['extract_flat'] not in (
                            'false', 'False', False)
                    if 'release_group' in wnt and wnt['release_group']:
                        ser['release_group'] = wnt['release_group']
                    if 'offset' in wnt:
    ...
    def ytdl_eps_search_opts(self, regextitle, playlistreverse, cookies=None, username=None,
                             password=None, rules=DEFAULT_RULES, extract_flat=True):
                ...
                for e, eps in enumerate(episodes):
                    if ser['id'] == eps['seriesId']:
                        ...
                        ydleps = self.ytdl_eps_search_opts(
                            ...
                            password, rules, ser.get('extract_flat', True))
